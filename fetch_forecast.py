...
def fetch_forecast(start_time, end_time, lat, lng, api_key):
    ...
    response = requests.get(
        'https://api.stormglass.io/v2/weather/point', timeout=5,
        params={
            'lat': lat,
            'lng': lng,
...
def fetch_tide(start_time, end_time, lat, lng, api_key):
    responce = requests.get(
        'https://api.stormglass.io/v2/tide/extremes/point', timeout=5,
        params={
            'lat': lat,
            'lng': lng,
            'start': start_time.to('UTC').timestamp(), # Convert to UTC timestamp
...
def store_forecast(path, latest_forecast):
    if 'errors' not in latest_forecast:#TODO add to a historical forecasts file/list
        ...
        error_string = ""
        json.dump(latest_forecast['errors'], error_string)
        print(f"Failed updating the forcast for {path}:"\
              f"{error_string}")
        return False
...
def get_api_key():
    api_file = open('api_key', 'r', encoding='utf-8')
...
def update_tides(lat, lng, start_time, end_time, tide_path, api_key,
    ...
    with open(tide_path, 'r', encoding='utf-8') as file:
